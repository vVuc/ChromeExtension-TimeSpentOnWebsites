...
(() => {
  ...
  function getMainDomain(url) {
    try {
      ...
      const parsed = O(hostname);
      if ("domain" in parsed) {
        return parsed.domain;
      }
      console.error("Erro ao analisar o dom\xEDnio:", parsed.error.message);
      return null;
    } catch (error) {
  ...
  // src/main.ts
  console.log("Atualizando storage para resolver um bug");
  chrome.storage.local.set({ timeSpentData: [] });
  async function veryfyAndInitializeStorageTimeSpentData() {
    const data = await chrome.storage.local.get("timeSpentData");
    if (!data.timeSpentData) {
      await chrome.storage.local.set({ timeSpentData: [] });
      return [];
    }
    console.log(data);
    return data.timeSpentData;
  }
  async function initializeStorageTempData() {
    let {
      tempData
    } = await chrome.storage.session.get("tempData");
    if (!tempData) {
      tempData = {
        lastActiveUrl: "",
        lastActiveSiteTabId: 0,
        timeSpentOnWebsites: (/* @__PURE__ */ new Date()).getTime()
      };
    }
  ...
  async function addNewSiteTimeSpent(params) {
    let timeSpentData = await chrome.storage.local.get("timeSpentData").then((data) => data.timeSpentData || []);
    timeSpentData.push(params);
    chrome.storage.local.set({ timeSpentData });
  }
  async function getStoredTimeSpentData() {
    const data = await veryfyAndInitializeStorageTimeSpentData();
    console.log(data);
    return data;
  }
  chrome.tabs.onActivated.addListener(async (activeInfo) => {
    const arrObj = await getStoredTimeSpentData();
    const { lastActiveUrl, lastActiveSiteTabId, timeSpentOnWebsites } = await initializeStorageTempData();
    let tab = await chrome.tabs.get(activeInfo.tabId);
    if (!tab.url || !tab.title || !tab.favIconUrl) {
      console.log("A pagina n\xE3o carregou os dados basicos ainda");
      return;
    }
    console.log("getMainDomain", getMainDomain(tab.url));
    const tabUrl = getMainDomain(tab.url) ?? "Url Indefinida";
    const title = tab.title;
    const icon = tab.favIconUrl;
    console.log("tabUrl ", tabUrl);
    const dataSite = { site: { url: tabUrl, title, icon }, timeSpent: 0, tabId: activeInfo.tabId };
    if (arrObj.length === 0) {
      chrome.storage.local.set({ timeSpentData: [dataSite] });
    ...
    if (existingSite) {
      console.log("Site j\xE1 existe no array, n\xE3o adicionando novamente.");
    } else {
      await addNewSiteTimeSpent(dataSite);
    }
